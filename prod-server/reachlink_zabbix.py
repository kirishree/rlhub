...
def tunnel_ip_info():
    ...
    try:
        ...
        tunnelIPs = list(coll_tunnel_ip.find({}))  # Convert cursor to list
            ...
            for device in organization.get("registered_devices", []):  # Use .get() to avoid KeyError
                ...
                for spokeip in tunnelIPs:
                    if spokeip.get("uuid") == device.get("uuid"):  # Use .get() for safe access
                        ...
                        matched_spokeip["spokedevice_name"] = device.get("spokedevice_name")
    ...
    while(iteration):
        ...
        if is_empty:
            for device in reachlink_current_info:
                print(device)
                new_host = device["spokedevice_name"]
                new_ip = device["tunnel_ip"]
            ...
            for device in reachlink_current_info:
                ...
                for device_zabbix in coll_reachlink_zabbix_info.find({},{'_id':0}):
                    if device['spokedevice_name'] == device_zabbix['Host_name']:
                        new_device = 0
                        if device['tunnel_ip'] != device_zabbix['Host_ip']:
                            ...
                            if status == True:
                                ...
                                update = {"$set": {"Host_ip": device['tunnel_ip']}}
                                coll_reachlink_zabbix_info.update_one(filter, update)
                            else:
                                print(f"Error in updating Tunnel IP address of host: {device['public_ip']} ")
