...
@csrf_exempt
def addstaticroute_hub_ubuntu(request: HttpRequest):
    try:
        data = json.loads(request.body)         
...
#            interface_addresses = configured_address_interface()
        ...
        if len(real_routes) > 0:
            ...
            background_thread.start() 
            #pbrresponse = configurepbr_spoke(pbr_spoke_data)

        if len(invalid_routes) == 0: 
            response = {"message":f"Successfully added {len(data['routes_info'])} subnet(s)."}    
        else:
            added_subnet = len(data['routes_info']) - len(invalid_routes)
            if added_subnet == 0:
                response = {"message":f"{invalid_routes} is(are) already routed."}
            else:
                response = {"message":f"Successfully added {added_subnet} subnet(s). {invalid_routes} is already routed."}
    except Exception as e:    
        response = {"message": f"Error in adding route, pl try again {e}" }
    logger.debug(f'Received request: {request.method} {request.path}')   
    print(response) 
        ...
        if organization_id:            
            ...
            if details:                                                   
                if current_datetime < details["subscription_to"]:
                    registered_devices_info = details["registered_devices"]                  
                    for device in registered_devices_info:
                        if device['uuid'].lower() == data["uuid"].lower():  
