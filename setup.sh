...
mv /etc/reach/reachlink.service /etc/systemd/system/
mv /etc/reach/reachlink_test.service /etc/systemd/system/
cd /etc/reach/reachlink
pip install -r requirements.txt
python3 manage.py migrate
#python3 db_config.py
#python3 tunnel_create.py
#chmod +x tunnel_create_bootup.py
cd "$original_dir"
sudo systemctl daemon-reload
sudo systemctl enable reachlink
sudo systemctl start reachlink
sudo systemctl enable reachlink_test
sudo systemctl start reachlink_test
#echo "python3 /etc/reach/reachlink/tunnel_create_bootup.py" >> /root/.bashrc
...
sudo useradd --system --no-create-home --shell /usr/sbin/nologin reachlinkfe
sudo mkdir -p /opt/reachlinkfe
#Clone the etel_reachlink_web_fe
sudo chown -R reachlinkfe:reachlinkfe /opt/reachlinkfe
cd "$original_dir" || exit

#Modify the settings .py for rest settings. & python3 -c "import secrets; print(secrets.token_urlsafe(64))"
#export SECRET_KEY_JWT="HZPccvJRyk7B22KHkUl_PFVd-I1KYp2J6UtueMlukgK9kIqSOJGBT1wXpPpveYnukMzKectYhdYkS9sQRDaE2Q" in reachlink folder