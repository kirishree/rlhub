...
def interfaceconfig(data):   
   # Define the router details       
    ...
    try:
        ...
        for i in range(1, len(addresses_info)):
            addr = addresses_info[i]
            addr = addr.strip()
                # Clean up extra spaces or non-visible characters using regex
    ...
    try:
        try:
            ...
            extra={
                "device_type": "Microtek",
                "device_ip": router_ip,
                "be_api_endpoint": "get_firewall_nat_details",
                "exception": str(e)
            }
            )
            return []
        try:
            # Execute the trace command 
            stdin, stdout, stderr = ssh_client.exec_command(f'/ip firewall nat print detail')
            # Initialize variables for output collection
            start_time = time.time()
            timeout = 10  # Stop after 10 seconds
...
def addapp(data):   
    ...
    try:
        ...
        response = [{"message":"Error while adding app for blocking. Pl try again!"}] 
    finally:
        # Close the SSH connection
        ssh_client.close()        
        return response
    ...
    try:
        ...
        for rule in data["rules"]:   
            if rule["protocol"] == "any" or rule["protocol"] == "all":    
                stdin, stdout, stderr = ssh_client.exec_command(f'/ip firewall filter add action={rule["action"]} src-address={rule["src_address"]} dst-address={rule["dst_address"]} src-port={rule["src_port"]} dst-port={rule["dst_port"]} comment={rule["description"]}')     
            else:
                stdin, stdout, stderr = ssh_client.exec_command(f'/ip firewall filter add action={rule["action"]} src-address={rule["src_address"]} dst-address={rule["dst_address"]} src-port={rule["src_port"]} dst-port={rule["dst_port"]} protocol={rule["protocol"]} comment={rule["description"]}')
            # Read the actual output and errors
            #output = stdout.read().decode()
            #if output:               
