...
m2m_exe_path = config('M2M_EXE_PATH')
# Zabbix API URL
...
@api_view(['POST'])  
@permission_classes([IsAuthenticated])
def get_ratelimit_info(request):
    try:
        data = json.loads(request.body)
        #print(data)
        # Capture the public IP from the request headers
...
def edit_ratelimit(request):
    try:
        ...
        public_ip = request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR')
        logger.debug(f"Requested_ip:{public_ip}, payload: {data}",
                    extra={ "be_api_endpoint": "edit_ratelimit" }                    
                    )
        branch_id = data["tunnel_ip"].split("/")[0] 
        ...
        if ".net" not in data.get("uuid", ""):            
            ...
            data["router_username"] = router_info["router_username"]
            data["router_password"] = router_info["router_password"]
            response = microtek_configure.edit_rate_limit(data)                 
            return JsonResponse(response, safe=False) 
        elif "cisco" in data["uuid"]:            
            #router_info = coll_tunnel_ip.find_one({"uuid":data["uuid"]})
            data["router_username"] = router_info["router_username"]
...
def get_lan_clients(request):
    try:
        ...
        public_ip = request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR')
        logger.debug(f"Requested_ip:{public_ip}, payload: {data}",
                    extra={ "be_api_endpoint": "get_lan_clients" }                    
                    )
        branch_id = data["tunnel_ip"].split("/")[0] 
        if ".net" in data.get("uuid", ""):       
            cache1_key = f"branch_details_{data['uuid']}"
        ...
        if ".net" not in data.get("uuid", ""):            
            ...
            data["router_username"] = router_info["router_username"]
            data["router_password"] = router_info["router_password"]
            response = microtek_configure.get_lan_clients_info(data)          
            return JsonResponse(response, safe=False, status=200) 
        elif "cisco" in data["uuid"]:            
            #router_info = coll_tunnel_ip.find_one({"uuid":data["uuid"]})
            data["router_username"] = router_info["router_username"]
...
def add_ratelimit(request):
    try:
        ...
        public_ip = request.META.get('HTTP_X_FORWARDED_FOR') or request.META.get('REMOTE_ADDR')
        logger.debug(f"Requested_ip:{public_ip}, payload: {data}",
                    extra={ "be_api_endpoint": "add_ratelimit" }                    
                    )
        branch_id = data["tunnel_ip"].split("/")[0] 
        cache_key = f"ratelimit_branch_{branch_id}"      
        cache.delete(cache_key)
        ...
        if ".net" not in data.get("uuid", ""):            
            ...
            data["router_username"] = router_info["router_username"]
            data["router_password"] = router_info["router_password"]
            response = microtek_configure.add_rate_limit(data)                 
            return JsonResponse(response, safe=False) 
        elif "cisco" in data["uuid"]:            
            #router_info = coll_tunnel_ip.find_one({"uuid":data["uuid"]})
            data["router_username"] = router_info["router_username"]
...
def del_ratelimit(request):
    try:
        ...
        if ".net" not in data.get("uuid", ""):            
            ...
            data["router_username"] = router_info["router_username"]
            data["router_password"] = router_info["router_password"]
            response = microtek_configure.del_rate_limit(data)                 
            return JsonResponse(response, safe=False) 
        elif "cisco" in data["uuid"]:            
            #router_info = coll_tunnel_ip.find_one({"uuid":data["uuid"]})
            data["router_username"] = router_info["router_username"]
